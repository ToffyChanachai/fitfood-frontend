    ...
    <div class="mt-[-20px]">
        <div class="flex space-x-2 items-center relative">
            <div class="mt-4 px-4 flex items-center space-x-1 mr-auto ">
                ...
                <span class="text-m text-gray-700">จำนวนประเภทเมนูทั้งหมด: </span>
                <span class="text-m text-custom-orange font-bold"> {{ meal_types.length }} รายการ</span>
            </div>

            <button v-if="selectedMenuType.length > 0" @click="clearFilter"
                class="px-2 py-2 rounded-md flex items-center space-x-1 text-gray-400 hover:text-custom-orange">
                <span class="material-symbols-outlined">close</span>
                <span class="ml-2">รีเซ็ตตัวกรอง ({{ selectedMenuType.length }})</span>
            </button>
                        ...
                        <div class="p-6 space-y-4 ">
                            ...
                            <div>
                                <label for="promotionType"
                                    class="block font-bold text-gray-700">ประเภทโปรแกรมเมนู</label>
                                <multiselect v-model="newMealType.menu_type_id" :options="menu_types" :multiple="false"
                                    :searchable="true" :close-on-select="true" placeholder="เลือกประเภทโปรแกรมเมนู"
                                    label="name" track-by="id" class="z-50" /> <!-- add z-50 here -->
                            </div>
                        </div>
            ...
            <div class="filter relative inline-block" ref="filterDropdown">
                ...
                <div v-if="isFilterDropdownOpen"
                    ...
                    <div class="p-4 w-[500px] list-none">
                        <h3 class="font-bold mb-2">กรองโดยประเภทโปรแกรมเมนู</h3>
                        <div class="grid grid-cols-3 gap-4">
                            <label v-for="type in menu_types" :key="type.id" class="flex items-center space-x-2">
                                <!-- Custom checkbox -->
                                <input type="checkbox" v-model="selectedMenuType" :value="type.id"
            ...
            <div v-if="isEditModalOpen"
                ...
                <div class="bg-white rounded-md shadow-lg w-1/2 max-w-3xl h-auto max-h-[800px] flex flex-col">
                    <div class="flex justify-between items-center bg-custom-orange text-white px-4 py-2 rounded-t-md">
                        <span class="font-bold">แก้ไขประเภทเมนู</span>
                        <div class="flex space-x-2">
                    ...
                    <div class="p-6 space-y-4">
                        <div class="mb-4">
                            ...
                            <input v-model="selectedMealType.name" id="editName" type="text"
                                class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-custom-orange" />
                        </div>

                        <div class="mb-4">
                            <label for="editPromotionType"
                                class="block font-bold text-gray-700">ประเภทโปรแกรมเมนู</label>
                            <multiselect v-model="selectedMealType.menu_type_id" :options="menu_types" :track-by="'id'"
                                :label="'name'" placeholder="เลือกประเภทโปรแกรมเมนู" />
                        </div>
                    ...
                    <div class="flex justify-end space-x-4 p-4 bg-white border-t rounded-b-md list-none">
                        <div class="flex space-x-2">
                            ...
                            <button @click="saveChanges"
                                class="bg-custom-orange text-white px-4 py-2 rounded hover:bg-custom-orange-hover flex items-center space-x-30">
                                บันทึก
                            </button>
                        </div>
...
export default {
    ...
    data() {
        return {
            headers: ['#', 'ชื่อ', 'ประเภทโปรแกรมเมนู', ''],
    ...
    computed: {
        totalPages() {
            return Math.ceil(
                this.meal_types.filter((meal_type) => {
    ...
    methods: {
        ...
        applyFilter() {
            if (this.selectedMenuType.length > 0) {
                this.filteredMealType = this.meal_types.filter(meal_type =>
                    this.selectedMenuType.includes(meal_type.menu_type_id)
                );
            } else {
        ...
        getMenuTypeName(menu_type_id) {
            const type = this.menu_types.find(type => type.id === menu_type_id);
            return type ? type.name : 'ไม่พบข้อมูล';
        },

        openEditModal(meal_type) {
            this.selectedMealType = {
                ...meal_type,
                menu_type_id: this.menu_types.find(
                    (type) => type.id === meal_type.menu_type_id
                ),
            };
        ...
        async saveChanges() {
            try {
                if (!this.selectedMealType.name || !this.selectedMealType.menu_type_id) {
        ...
        async addMealType() {
            if (!this.newMealType.name || !this.newMealType.menu_type_id) {
