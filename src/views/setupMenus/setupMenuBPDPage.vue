...
export default {
  ...
  computed: {
    filteredMealTypes() {
      return this.meal_types.filter(
        (item) => item.menuType && item.menuType.id === 15
      );
    },
    // filteredMenus() {
  ...
  methods: {
    async fetchSetupMenus() {
      try {
        const response = await fetch("http://127.0.0.1:3333/setup-menu-bpd");
    ...
    async fetchSetupMenusForToday() {
      try {
        ...
        const response = await axios.get(
          `http://127.0.0.1:3333/setup-menu-bpd/menus-by-day/${todayDate}`
        );
    ...
    async fetchMenusByDay() {
      ...
      try {
        const response = await axios.get(
          `http://127.0.0.1:3333/setup-menu-bpd/menus-by-day/${this.selectedDate}`
        );
    ...
    async fetchStartDate() {
      try {
        const response = await axios.get(
          "http://127.0.0.1:3333/setup-menu-bpd/get-start-date"
        );
    ...
    async updateStartDate() {
      try {
        ...
        const response = await axios.post(
          "http://127.0.0.1:3333/setup-menu-bpd/set-start-date",
          {
    ...
    getFilteredMenus(selectedMealType) {
      ...
      return this.menus.filter((menu) => {
        ...
        return mealType && mealType.menuType.id === 15;
      });
    },
    ...
    async addMenus() {
      try {
        ...
        await axios.post("http://127.0.0.1:3333/setup-menu-bpd", {
    ...
    async saveChanges() {
      try {
        ...
        await axios.put(
          `http://127.0.0.1:3333/setup-menu-bpd/${this.selectedSetupMenu.id}`,
          {
        ...
        // const response = await axios.put(`http://127.0.0.1:3333/setup-menu-bpd/${this.selectedSetupMenu.id}`, {
        //   day_of_week: this.selectedSetupMenu.day_of_week,
        //   menu_id: this.selectedSetupMenu.menu_id.id,
        // });
    ...
    async deleteConfirmed() {
      try {
        await axios.delete(
          `http://127.0.0.1:3333/setup-menu-bpd/${this.itemToDelete}`
        );
