    ...
    <div class="flex justify-end items-center space-x-2 relative">
        ...
        <div class="flex items-center space-x-1 mr-auto ">
            <button @click="viewMode = 'week'"
                :class="['px-2 py-1 rounded-lg  hover:bg-gray-50', { ' text-custom-orange': viewMode === 'week' }]">
                <span class="material-symbols-outlined text-2xl">calendar_view_week</span>
            </button>
            <button @click="viewMode = 'day'"
                :class="['px-2 py-1 rounded-lg  hover:bg-gray-50', { ' text-custom-orange': viewMode === 'day' }]">
                <span class="material-symbols-outlined text-2xl">calendar_view_day</span>
            </button>

            <p class="text-4xl mb-2">|</p>
            <h1 class="text-2xl font-bold mb-2 ml-auto">{{ viewMode === 'week' ? 'ดูตามสัปดาห์' : 'ดูตามวัน' }}</h1>
        </div>
        ...
        <div class="flex items-center space-x-4">
            ...
            <button @click="updateStartDate"
                class="bg-custom-orange text-white px-2 py-1 rounded flex items-center space-x-1 hover:bg-custom-orange-hover"
                    ...
                    <div class="p-6 space-y-4 overflow-y-auto flex-grow">
                        ...
                        <div v-for="(item, index) in formRows" :key="index" class="space-y-4 relative">
                            ...
                            <div>
                                ...
                                <multiselect v-model="item.selectedMealType" :options="filteredMealTypes"
                                    :multiple="false" :searchable="true" :close-on-select="true"
                                    placeholder="เลือกประเภทอาหาร" label="name" track-by="id" />
                            </div>

                            <!-- เมนู -->
                            <div>
                                <label for="menus" class="block font-bold text-gray-700">เมนู</label>
                                <multiselect v-model="item.selectedMenus"
                                    :options="getFilteredMenus(item.selectedMealType)" :multiple="true"
                    ...
                    <div class="flex justify-between space-x-4 p-4 bg-white border-t rounded-b-md list-none">
                        ...
                        <div class="flex space-x-2">
                            <button @click="closeAddModal"
                                class="px-4 py-2 rounded bg-gray-300 hover:bg-gray-400 text-gray-700">ยกเลิก</button>
                            <button @click="addMenus"
                                class="px-4 py-2 rounded bg-custom-orange text-white hover:bg-custom-orange-hover">บันทึก</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div v-show="viewMode === 'week'">
        <div class="flex items-center space-x-3 py-2">
            ...
            <multiselect v-if="weekOptions.length > 0" v-model="selectedWeek" :options="weekOptions"
                placeholder="เลือกสัปดาห์" :multiple="false" :taggable="true" :close-on-select="true"
                :allow-empty="true" class="w-60">
            </multiselect>
            ...
            <table class="min-w-full table-auto rounded-t-2xl overflow-hidden mt-4">
                ...
                <tbody>
                    <tr v-for="(menu, index) in paginatedMenus" :key="menu.id"
                        ...
                        <td v-if="index === 0 || menu.day_of_week !== paginatedMenus[index - 1].day_of_week"
                        ...
                        <td class="px-4 py-2 align-top pb-5">{{ getMealTypeName(getMealTypeID(menu.menu_id)) }}</td>
                        <td class="px-4 py-2 align-top pb-5">{{ getMenuEnglishName(menu.menu_id) }}</td>
                        <td class="px-4 py-2 align-top pb-5">{{ getMenuThaiName(menu.menu_id) }}</td>
                    ...
                    <tr v-if="paginatedMenus.length === 0">
                        <td colspan="6" class="py-10 bg-white text-center text-gray-500 font-bold">ไม่พบข้อมูล</td>
                    </tr>
                </tbody>
            </table>
    ...
    <div v-show="viewMode === 'day'">
        ...
        <table class="min-w-full table-auto rounded-t-2xl overflow-hidden mt-4">
            ...
            <tbody>
                ...
                <tr v-if="paginatedMenusByDay.length === 0">
                    <td colspan="5" class="py-10 bg-white text-center text-gray-500 font-bold">ไม่พบข้อมูล</td>
                </tr>
            </tbody>
        </table>
        ...
        <div class="rounded-b-2xl flex justify-center items-center space-x-2 bg-white px-2 py-1">
            ...
            <div class="flex items-center space-x-1">
                ...
                <!-- ปุ่มสำหรับหน้าแต่ละหน้า -->
                <button v-for="page in totalPagesArrayByDay.range" :key="page" @click="goToPageByDay(page)"
                    :class="['px-3 py-2 rounded-md', { 'bg-custom-orange text-white': currentPage === page, 'bg-white': currentPage !== page }]"
                    class="cursor-pointer hover:bg-custom-orange hover:text-white">
                    {{ page }}
                </button>
        ...
        <div class="bg-white rounded-md shadow-lg w-1/2 max-w-3xl h-auto max-h-[800px] flex flex-col">
            ...
            <div class="p-6 space-y-4">
                <div class="flex items-center space-x-4">
                    ...
                    <input id="editDayofWeek" v-model="selectedSetupMenu.day_of_week" type="number"
...
export default {
    data() {
        return {
            ...
            formRows: [
                { selectedMealType: null, selectedMenus: [] }
            ],
            selectedSetupMenu: {
                id: '',
                day_of_week: "",
    ...
    computed: {
        filteredMealTypes() {
            return this.meal_types.filter(item => item.menuType && item.menuType.name === 'PH');
        },
        // filteredMenus() {
        ...
        //     return selectedMealType && selectedMealType.menuType.name === 'PH';
        //   });
        // },
        ...
        filteredMenus() {
            ...
            return menus.filter(menu => {
                const englishName = this.getMenuEnglishName(menu.menu_id) ? this.getMenuEnglishName(menu.menu_id).toLowerCase() : '';
                const thaiName = this.getMenuThaiName(menu.menu_id) ? this.getMenuThaiName(menu.menu_id).toLowerCase() : '';
                const search = this.searchQuery.toLowerCase();

                return englishName.includes(search) || thaiName.includes(search) || menu.day_of_week.toString().includes(search);
            });
        },
        filteredMenusByDay() {
        ...
        filteredEditMenus() {
            ...
            return this.menus.filter(menu => menu.meal_type_id === this.selectedSetupMenu.meal_type_id.id);
        },
        menuLabel() {
    ...
    methods: {
        ...
        async fetchSetupMenusForToday() {
            try {
                ...
                const response = await axios.get(`http://127.0.0.1:3333/setup-menu-ph/menus-by-day/${todayDate}`);
        ...
        async fetchStartDate() {
            try {
                const response = await axios.get("http://127.0.0.1:3333/setup-menu-ph/get-start-date");
        ...
        async updateStartDate() {
            try {
                ...
                const response = await axios.post("http://127.0.0.1:3333/setup-menu-ph/set-start-date", {
                    startDate: this.startDate,
                });

                // console.log("วันที่ที่เลือก: ", this.startDate);
        ...
        getFilteredMenus(selectedMealType) {
            if (selectedMealType) {
                return this.menus.filter(menu => menu.meal_type_id === selectedMealType.id);
            }
            return this.menus.filter(menu => {
                const mealType = this.meal_types.find(item => item.id === menu.meal_type_id);
                return mealType && mealType.menuType.name === 'PH';
            });
        },
        ...
        async addMenus() {
            try {
                const menus = this.formRows.map(row => row.selectedMenus.map(menu => ({ menu_id: menu.id }))).flat();
        ...
        openEditModal(menu) {
            ...
            this.selectedSetupMenu = {
                ...menu,
                menu_id: selectedMenu ? { id: selectedMenu.id, name_thai: selectedMenu.name_thai } : null,
                meal_type_id: this.meal_types.find((mt) => mt.id === selectedMenu.meal_type_id) || null,
            };
        ...
        async saveChanges() {
            try {
                if (!this.selectedSetupMenu.day_of_week && !this.selectedSetupMenu.menu_id) {
                    this.showErrorToastNotification("กรุณากรอกข้อมูลให้ครบถ้วน!");
                    return;
                }
                ...
                await axios.put(`http://127.0.0.1:3333/setup-menu-ph/${this.selectedSetupMenu.id}`, {
                    day_of_week: this.selectedSetupMenu.day_of_week,
                    menu_id: this.selectedSetupMenu.menu_id.id,
                });

                // const response = await axios.put(`http://127.0.0.1:3333/setup-menu-ph/${this.selectedSetupMenu.id}`, {
                //   day_of_week: this.selectedSetupMenu.day_of_week,
                //   menu_id: this.selectedSetupMenu.menu_id.id,
                // });
        ...
        async deleteConfirmed() {
            try {
                await axios.delete(`http://127.0.0.1:3333/setup-menu-ph/${this.itemToDelete}`);
