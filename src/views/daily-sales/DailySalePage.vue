...
<tr class="bg-white font-bold">
    ...
    <td class="px-4 py-2">{{ formatPrice(totalPreVatSum) }}</td>
        ...
        <div class="mt-2 flex space-x-4">
            ...
            <div class="w-1/2">
                <h1 class="text-xl font-bold mb-2">Additional Sales</h1>
                <table class="w-full table-auto rounded-2xl overflow-hidden">
                    <thead>
            ...
            <div class="w-1/2">
                <h1 class="text-xl font-bold mb-2">วิธีการชำระเงิน</h1>
                <table class="w-full table-auto rounded-2xl overflow-hidden">
                    <thead>
        ...
        <h1 class="text-xl font-bold mt-4 mb-2">Package Types</h1>
...
export default {
    ...
    data() {
        return {
            headers: ['Transaction Date', `Customer's Name`, 'Package Type', 'Sales Category', 'Package/Purchase Details', 'Package Price', 'Details', 'Additional Discount', '% Extra Charge', 'Extra Charge Value', 'Purchase Value', 'ค่าจัดส่งรวม', 'ยอดขายรวม', 'วิธีการชำระเงิน', 'Sales Rep'],
            headerWidths: ['100px', '500px', '300px', '300px', '600px', '100px', '600px', '100px', '100px', '100px', '100px', '100px', '100px', '200px', '100px'],
        ...
        filteredSalesBySellerAddit() {
            return Object.keys(this.salesBySellerAndPackageType).map(sellerId => {
                const filteredSales = this.salesBySellerAndPackageType[sellerId].sales.filter(sale => {
                    const packageTypeName = this.getPackageTypeName(sale.package_type_id);
                    const name = packageTypeName ? packageTypeName.toLowerCase() : '';
                    return name === 'additional sales' || name === 'consignment';
                });
