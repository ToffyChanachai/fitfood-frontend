<template>
  ...
  {{ formatDate(order.order_date) }}
    ...
    <div ref="componentRef" class="hidden-print p-12 relative">
      <div class="relative flex justify-between items-center p-4">
        <strong class="text-xl">Kitchen Orders Report</strong>
        <img src="@/assets/logo_fitfood_full.png" alt="Logo" class="w-50 h-20" />
      </div>


      <div v-for="(orders, index) in groupedOrdersPrint" :key="index" class="mb-4 mt-2">
...
export default {
  setup() {
    // ใช้ VueToPrint กับ Composition API
    const componentRef = ref();
    const { handlePrint } = useVueToPrint({
      content: componentRef, // กำหนดเนื้อหาที่จะพิมพ์
      documentTitle: "AwesomeFileName", // ชื่อเอกสารที่พิมพ์
    });
  ...
  computed: {
    ...
    groupedOrdersPrint() {
      const groupedByMenuTypeAndDate = this.orders.reduce((acc, order) => {
        ...
        // ตรวจสอบว่า menu_type_id มีอยู่ใน acc หรือไม่
        if (!acc[menuType]) {
          acc[menuType] = [];
        }

        // ตรวจสอบว่า order_date มีอยู่ใน menu_type นี้หรือไม่
      ...
      // แปลงจาก object ที่ group ตาม menu_type_id เป็น array
